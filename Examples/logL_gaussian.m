...
%     NOTE: C can be a single number (the noise variance), an vector of variance
%           values for each data point, or a full covariance matrix
...
if (size(C,1)==1 || N == 1) && length(C) == 1
    % variance is just a single value
    invC = 1/C;
    lDetC = N*log(C);
elseif size(C,1) == 1 && size(C,2) == N
    % variance is a vector of values
    Ctemp = eye(N);
    % set diagonal elements
    Ctemp(logical(eye(N))) = C;
    invC = inv(Ctemp);
    lDetC = log(det(Ctemp));
